...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='pacglobal.conf'
CONFIGFOLDER='/root/.PACGlobal'
COIN_DAEMON='pacglobald'
COIN_CLI='pacglobal-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/PACGlobalOfficial/PAC/releases/download/8efaee16f/pacglobal-8efaee16f-lin64.tgz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='pacglobal'
COIN_PORT=7112
RPC_PORT=7113
BOOTSTRAP='https://www.dropbox.com/s/oaxp7875wy8kxpp/pac_bootstrap.zip'
...
function download_bootstrap() {
  ...
  rm -rf $CONFIGFOLDER/evodb >/dev/null 2>&1
  rm -rf $CONFIGFOLDER/llmq >/dev/null 2>&1
...
function create_key() {
    echo -e "Enter your $COIN_NAME masternodeblsprivkey secret Key. Or Press enter generate to make the system generate one new key"
    read -t 1 -e COINKEY
    if [[ -z "$COINKEY" ]]; then
        $COIN_PATH$COIN_DAEMON -daemon
        while [[ ! $($COIN_PATH$COIN_CLI getblockcount 2> /dev/null) =~ ^[0-9]+$ ]]; do
            sleep 1
        done
        if [ -z "$(ps axo cmd:100 | grep $COIN_PATH$COIN_DAEMON)" ]; then
        ...
        COINKEY=$($COIN_PATH$COIN_CLI bls generate)
        if [ "$?" -gt "0" ]; then
            echo -e "Wallet not fully loaded. Let us wait and try again to generate the  Key"
            echo -e "{\"error\":\"Wallet not fully loaded. Let us wait and try again to generate the  Key.\",\"errcode\":1099}"
            while [[ ! $($COIN_PATH$COIN_CLI getblockcount 2> /dev/null) =~ ^[0-9]+$ ]]; do
                sleep 1
            done
            COINKEY=$($COIN_PATH$COIN_CLI bls generate)
        fi
        SECRET_KEY=$(node -e 'console.log(JSON.parse(process.argv[1]).secret);' "$COINKEY")
        PUBLIC_KEY=$(node -e 'console.log(JSON.parse(process.argv[1]).public);' "$COINKEY")
