...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='smartcash.conf'
CONFIGFOLDER='/root/.smartcash'
COIN_DAEMON='smartcashd'
COIN_CLI='smartcash-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/SmartCash/Core-Smart/releases/download/v1.2.8/smartcash-1.2.8-x86_64-linux-gnu.tar.gz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='smartcash'
COIN_PORT=9678
RPC_PORT=9679
BOOTSTRAP='https://smartcash.cc/txindexstrap.zip'
...
function download_bootstrap() {
  ...
  rm -rf $CONFIGFOLDER/rewards >/dev/null 2>&1
...
function create_key() {
  echo -e "${YELLOW}Enter your ${RED}$COIN_NAME SmartNode GEN Key${NC}. Or Press enter to generate a new Genkey"
  read -t 10 -e COINKEY
  if [[ -z "$COINKEY" ]]; then
  ...
  COINKEY=$(try_cmd $COIN_PATH$COIN_CLI "createmasternodekey" "smartnode genkey")
  if [ "$?" -gt "0" ];
    then
