...
KERN_ARCH=$(uname -m)
COIN_TGZ="https://github.com/Deviantcoin/Source/releases/download/4.0.0.0/deviant-4.0.0-x86_64-linux-gnu.tar.gz"
...
if [ -n "$(pidof Deviantd)" ] || [ -e "Deviantd" ] ; then
  echo -e "${RED}Old Deviantd is already installed - removing previous installation.${NC}"
  systemctl stop DeviantCore.service
  systemctl disable DeviantCore.service
  sleep 3
  rm /etc/systemd/system/DeviantCore.service
  systemctl daemon-reload
fi
}
