...
function cmd_mpstat() {
	cmd=$(mpstat -P ALL 1 1 | sed '/Average/!d')
	echo -e "$cmd"
	local rows=($(echo "$cmd" | awk '{print $1}' | tail -n +2))
	local js="{ "
	for (( i=0; i<${#rows[@]}; i++ )); do
		js+=" \"$(matrix_get $(($i+1)) 1)\": $(bc <<< 100-$(matrix_get $(($i+1)) 11)), "
	done
	json_finish "$js"
}
function cmd_free() {
...
case "$1" in
	...
	*)
		...
		echo -e "Currently accepted commands: free, mpstat, df"
		;;
esac
